...
fig = go.Figure()
tmax = go.Scatter(x=df["time"], y=df["0"], mode="markers", name="Pontos de temperatura máxima diária")
...
for i in range(0, n):
    for mes in range(1,13):
        ...
        k+=1
fig = go.Figure()
tmax = go.Scatter(x=df["time"], y=df["0"], mode="markers", name="Pontos de temperatura máxima mensal")
...
fig = go.Figure( data=[tmax, txx_line], layout=lay )
fig.write_html("TXx_mensal.html")
